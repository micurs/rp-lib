 ...
 */
export const concat = <I1, I2>(secondSource$: Observable<I2>): Operator<I1, I1 | I2> =>
(firstSource$: Observable<I1>): Observable<I1 | I2> => {
  const result$ = new Subject<I1 | I2>();
  const secondSourceCache: Array<I2> = [];

  // Subscribe to the second observable and cache its values until the first observable completes
  ...
  // Subscribe to the first observable and emit its values to the result observable
  // when it completes, we emit all cached values from the second observable
